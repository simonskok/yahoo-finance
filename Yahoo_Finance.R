...
Yahoo_Daily_Gainers <- function (N = 20) 
{
    ...
    Gainers <- Gainers$finance$result$quotes[[1]] %>% dplyr::filter(grepl("Real", 
        quoteSourceName)) %>% dplyr::mutate_at(dplyr::vars(dplyr::contains("Time")), 
...
Yahoo_News_History_Ticker <- function (Ticker, count = 50) 
{
...
Upcoming_Earnings_Release <- function (Ticker) 
{
...
Half_Open_Days <- function () 
as.Date(c("2023-07-03", "2024-07-03", "2025-07-03", "2023-11-24", 
    "2024-11-29", "2025-11-28", "2024-12-24", "2025-12-24")) %>% 
    sort
...
Last_Friday <- function () 
Sys.Date() - lubridate::wday(Sys.Date() + 1)
